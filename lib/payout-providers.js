...
class MobileMoneyPayoutProvider extends PayoutProvider {
  constructor(env = process.env) {
    super('MOMO');
    this.apiBaseUrl = String(env.MOMO_API_BASE_URL || '').trim();
    this.apiKey = String(env.MOMO_API_KEY || '').trim();
    this.apiSecret = String(env.MOMO_API_SECRET || '').trim();
  }

  isConfigured() {
    return Boolean(this.apiBaseUrl && this.apiKey && this.apiSecret);
  }

  async initiate() {
    // Keep external transfer disabled until the production disbursement
    // contract and credentials are verified. Never simulate a successful payout.
    ...
    throw new Error('MOMO_PAYOUT_CONTRACT_NOT_VERIFIED');
  }
}

...
class MobileMoneyPayoutProvider extends PayoutProvider {
  ...
  async initiate(withdrawal) {
    ...
    if (response.status !== 202) {
      // Do not attach the provider response body to the thrown error.
      // Provider bodies may contain account or transaction details that
      // must not reach application logs.
      throw new Error('MOMO_TRANSFER_HTTP_' + response.status);
    }
...
function getPayoutProvider(methodCode, env = process.env) {
  ...
  // MTN MoMo and Roshan M-Paisa are distinct payment products.
  // Never route M-Paisa through the MTN MoMo API.
  if (code === 'MOMO') {
    return new MobileMoneyPayoutProvider(env);
  }

  // M-Paisa exists as a withdrawal method in the database, but its
  // production payout contract has not been implemented. Fail closed
  // instead of treating it as a configured manual payout provider.
  if (code === 'M-PAISA') {
    return new class extends PayoutProvider {
      constructor() { super('M-PAISA'); }
      isConfigured() { return false; }
      async initiate() {
        return { provider: this.code, mode: 'MANUAL_FALLBACK', status: 'NOT_CONFIGURED' };
      }
    }();
  }

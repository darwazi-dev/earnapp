...
class MobileMoneyPayoutProvider extends PayoutProvider {
  ...
  async initiate(withdrawal) {
    ...
    const account = String(withdrawal.account_details || '').trim();
    if (!account) throw new Error('MOMO_PAYEE_MISSING');
  ...
  async getStatus(referenceId) {
    ...
    if (!/^[0-9a-f-]{36}$/i.test(ref)) throw new Error('MOMO_REFERENCE_INVALID');
